  ...
  return (
    <div className={styles.wrapper}>
      <form onSubmit={handleSubmit(onSubmit)}>
        <Field
          label={t('login.form.taikhoan-label', 'Tài khoản')}
          invalid={!!errors.user}
          error={errors.user?.message}
        >
          <Input
            {...register('user', { required: t('login.form.taikhoan-required', 'Bắt buộc nhập tài khoản') })}
            id={usernameId}
            autoFocus
            autoCapitalize="none"
            placeholder={loginHint || t('login.form.taikhoan-placeholder', 'Nhập tài khoản')}
        ...
        >
          <PasswordField
            {...register('password', { required: t('login.form.matkhau-required', 'Bắt buộc nhập mật khẩu') })}
            id={passwordId}
