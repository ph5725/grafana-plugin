...
export const PageMap: React.FC<PanelProps<SimpleOptions>> = ({ width, height, options }) => {
  ...
  useEffect(() => {
    const loadMap = async () => {
      ...
      // const portalURL = portal_edit + '&token=' + token_edit || 'https://iotplatform.intelli.com.vn/portal';
      // esriConfig.default.portalUrl = portalURL;
      ...
      portal.authMode = 'immediate'; // đảm bảo không yêu cầu đăng nhập
      console.log(portal_edit);

      console.log('Using token:', token_edit || 'https://iotplatform.intelli.com.vn/portal');

      //@ts-ignore
      // esriRequest(`${portal_edit}/apps/mapviewer/index.html?webmap=${webmapId_edit}`, {
      //   query: {
      //     f: "json",
      //     token: token_edit
      //   },
